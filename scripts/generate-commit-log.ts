...
const parseNumStat = (hash: string): CommitFile[] => {
...
const getAreas = (files: CommitFile[]) => {
...
const getTags = (files: CommitFile[]) => {
...
const pickVerb = (subject: string, tags: string[]) => {
...
const buildStats = (files: CommitFile[]): CommitStats => {
...
const buildPlainNotes = (tags: string[], subject: string) => {
  const notes: string[] = []
  const lower = subject.toLowerCase()
...
const buildPlainImpact = (tags: string[]) => {
...
const commits: CommitLogEntry[] = logRaw
  .split("\x1e")
