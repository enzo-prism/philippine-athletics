...
import { ArrowRight, Building2, CalendarDays, Dumbbell, Footprints, Medal, ShieldCheck, Sparkles, Zap } from "lucide-react"
...
const quickLinks = [
  ...
  { href: "/competitions", label: "Competitions", description: "Upcoming meets and official fixtures", icon: CalendarDays },
]
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f7f8fb_0%,#eef1f6_100%)] text-foreground">
      ...
      <main className="page-shell page-stack py-6 sm:py-8 lg:py-10">
        <section className="home-hero-shell relative overflow-hidden border-l-4 border-l-accent">
          <div className="absolute inset-0 bg-[linear-gradient(130deg,rgba(15,39,69,0.08)_0%,rgba(15,39,69,0)_42%,rgba(181,18,43,0.06)_100%)]" aria-hidden="true" />

          <div className="home-hero-content">
            <div className="grid gap-8 xl:grid-cols-[1.25fr_1fr]">
              <div className="space-y-5">
                <p className="institutional-kicker">Philippine Athletics Platform</p>
                ...
                <p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
                  From grassroots clubs to national champions, discover athletes, verify sanctioned results, and align everyone on the same official story.
                </p>
                <div className="flex flex-wrap gap-3">
                  <Button asChild size="lg" className="bg-primary px-8 text-primary-foreground hover:bg-primary/92">
                    <Link href="/membership">
                      Become a Member
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
                  </Button>
                  <Button asChild size="lg" variant="outline" className="border-primary/30 px-8 text-primary hover:bg-primary/10">
              ...
              <Card className="home-panel-dark border-l-4 border-l-accent">
                <CardContent className="space-y-3 p-5">
                  ...
                  <div className="space-y-1.5">
                    <h2 className="text-lg font-semibold tracking-tight text-foreground">Athletics Directory</h2>
                    <p className="text-xs text-muted-foreground">Jump directly to the teams, athletes, and events shaping this season.</p>
                  </div>
        ...
        <section className="border-l-4 border-l-accent bg-card px-5 py-6 sm:px-7" data-testid="safe-sport-section">
          <div className="grid gap-5 lg:grid-cols-[1fr_1.2fr] lg:items-center">
            <div className="space-y-2">
              <p className="institutional-kicker">Safe Sport</p>
              <h2 className="text-3xl font-semibold text-primary font-accent">Athlete Safety Is Non-Negotiable</h2>
              <p className="text-sm text-muted-foreground">
                Safe Sport standards are built into recognition, coaching visibility, and institutional governance so families and athletes can trust every pathway.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { title: "Recognized Clubs", text: "Only verified clubs and programs are highlighted for public discovery." },
                { title: "Certified Coaches", text: "Coach credentials and compliance signals are surfaced where decisions are made." },
                { title: "Clear Reporting", text: "Sanctioned event pathways support transparent and accountable results intake." },
              ].map((item) => (
                <div key={item.title} className="border border-border bg-background px-4 py-4">
        ...
        <section className="home-feature-grid">
          {[
            { icon: Medal, title: "National rankings", href: "/rankings", description: "Follow top performers and official season leaders." },
              ...
              <Card className="h-full border-border bg-card py-0 transition hover:-translate-y-1 hover:border-accent/40 hover:shadow-md">
                <CardContent className="space-y-3 p-5">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold uppercase tracking-[0.14em] text-primary">{item.title}</p>
